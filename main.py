...
try:
    ...
    print(
        f"WARNING: Failed to initialize pygame.mixer: {type(e).__name__} - {e}. Using dummy mixer.")
...
if __name__ == "__main__":
    ...
    # 不正な形式のダミーファイル
    with open("bad_format.wav", "w") as f:
        f.write("This is not a WAV file")
    with open("bad_format.mid", "w") as f:
    ...
    assert os.path.exists(
        app.prepared_accompaniment_wav), "accompaniment.wav not created"
    assert midi_conversion_success and os.path.exists(
        app.vocal_midi_filepath), "vocal.mid not created"
    ...
    if app.show_karaoke_window():
        ...
        app.draw_pitch_bars(
            pygame.mixer.music.get_pos() / 1000.0)  # 手動で描画をトリガー
    ...
    if os.path.exists(app.prepared_vocal_wav):
        os.remove(app.prepared_vocal_wav)
    ...
    # vocal.wavだけ準備 (accはコピーされない)
    app.select_and_prepare_files("test_vocal_orig.wav", None)
    app.convert_vocal_to_midi()  # vocal.midは作られる
    if os.path.exists(app.prepared_accompaniment_wav):
        os.remove(app.prepared_accompaniment_wav)  # 念のため削除
    app.show_karaoke_window()

    # 3c. カラオケ起動時にvocal.midがない
    print("\n--- Test Case 3c: Karaoke launch with missing vocal.mid ---")
    app.select_and_prepare_files(
        "test_vocal_orig.wav", "test_acc_orig.wav")  # 両方準備
    if os.path.exists(app.vocal_midi_filepath):
        os.remove(app.vocal_midi_filepath)  # MIDIだけ削除
    app.show_karaoke_window()
    ...
    app.select_and_prepare_files(
        "test_vocal_orig.wav", "test_acc_orig.wav")  # 正常なWAV準備
    ...
    # --- クリーンアップ (テスト用ファイルを削除) ---
    if os.path.exists("test_vocal_orig.wav"):
        os.remove("test_vocal_orig.wav")
    if os.path.exists("test_acc_orig.wav"):
        os.remove("test_acc_orig.wav")
    if os.path.exists("bad_format.wav"):
        os.remove("bad_format.wav")
    if os.path.exists("bad_format.mid"):
        os.remove("bad_format.mid")
